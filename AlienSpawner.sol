...
mapping (address => uint) CoreBaseFind; ///#DEV Find which CoreBase Id belongs to corresponding ETH address
...
function GenerateCoreBase(_hp, _RegenRate, _interest, _value, _AlienCount, _levels, _LossCounter, _WinCounter) private {
    ...
    CoreBaseFind(_CoreId);
    GenerateCore(_hp, _value, _Core);
}

function StoreCoreBaseIds() public {
    StoreIds[]; // In need of a gas efficient method, otherwise costs players fortune
    StoreIds.push(_CoreId);
}
...
function CreateRandomAlien(_name, _AlienSub, _attack, _defence, _level, _armor, _AttackCooldown, _AmalgamationCooldown) private {
    uint AlienId = alien.push(AlienType(_name, _AlienSub, _attack, _defence, 5, 5, false, uint64(_AttackCooldown + _AmalgamationCooldown)) - 1); ///#DEV Add alien (Name, attack score, defence score, level, armor boost, cooldown between attacks, hybrid creation, start at index 0)
    AlienOwner[_AlienId];
